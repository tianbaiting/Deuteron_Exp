...
int ypol_show_p()
{
    ...
    std::string ylab = "#it{p}_{x}^{n} (MeV/c)";
    double xmin = -400, xmax = 400, ymin = -400, ymax = 400;

    std::string target = "Sn124";
    Int_t bmin = 5, bmax = 10;
    std::vector<std::string> gams{"050", "060", "070", "080"};
    std::vector<std::string> pols{"ynp", "ypn"};
        ...
        for (Int_t b = bmin; b <= bmax; b++)
        {
            for (std::string p : pols)
            {
                ...
                while (inputfile >> eventNo)
                {
                    ...
                    // {
                        // hist2D[nx-1][i]->Fill(pzp + pzn,  pzp - pzn);
                        // hist2D[b - bmin][i]->Fill(pzp + pzn,  pzp - pzn);
                        hist2D[nx-1][i]->Fill(pyp , pyn);
                        hist2D[b - bmin][i]->Fill(pyp , pyn);

                    // }
                }
 
                std::cout << "Total entries in hist2D[" << b - bmin << "][" << i << "]: " << hist2D[b - bmin][i]->GetEntries() << std::endl;
            }
        }
                       // 输出每个直方图的总事件数
        std::cout << "Total entries in hist2D[nx-1][" << i << "]: " << hist2D[nx-1][i]->GetEntries() << std::endl;
    }
