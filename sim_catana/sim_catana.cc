...
int main(int argc,char** argv)
{
  ...
  CATANADetectorConstruction* detector =
    new CATANADetectorConstruction();
  runManager->SetUserInitialization(detector);

  // set physics construction class
  G4String physName = "QGSP_INCLXX_XS"; // default: is it okay for GAMMA?
  ...
  simDataManager->RegistInitializer(new CATANASimDataInitializer);
  simDataManager->RegistConverter(new CATANASimDataConverter_TArtCATANACsI);

  simDataManager->FindInitializer("CATANASimDataInitializer")->
    SetDataStore(false);
  
  simDataManager->FindInitializer("BeamSimDataInitializer")->
    SetDataStore(false);
  simDataManager->FindInitializer("FragSimDataInitializer")->
    SetDataStore(false);
  ...
  G4VUserPrimaryGeneratorAction* userPrimaryGeneratorAction =
    new PrimaryGeneratorActionBasic;
