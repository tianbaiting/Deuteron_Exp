...
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
CATANASD::CATANASD(G4String name) : G4VSensitiveDetector(name)
{
  std::cout << "CATANASD constructed" << std::endl;
}
...
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void CATANASD::Initialize(G4HCofThisEvent* /*HCTE*/){;}

//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
G4bool CATANASD::ProcessHits(G4Step *aStep, G4TouchableHistory* /*R0hist*/)
{
  SimDataManager *sman = SimDataManager::GetSimDataManager();
  TClonesArray *SimDataArray = sman->FindSimDataArray("CATANASimData");
  ...
  for(Int_t i=0;i<theTouchable->GetHistoryDepth();++i){
    Int_t depth = i; //0:self, 1:mother, 2:grandmother...
...
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void CATANASD::EndOfEvent(G4HCofThisEvent* /*HCTE*/){;}
