...
G4VPhysicalVolume* DayoneDetectorConstruction::Construct()
{
  ...
  if (fTargetMat == "empty")    TargetMaterial = WorldMaterial;
    ...
    G4Exception("DayoneDetectorConstruction::Construct()", "material", JustWarning, ed);
  }
  ...
  TDeutSimParameter *deut_prm = (TDeutSimParameter*)simDataManager->FindParameter("DeutParameter");
  if (deut_prm==0) {
    deut_prm = new TDeutSimParameter("DeutParameter");
    simDataManager->AddParameter(deut_prm);
  }
  ...
  if (fSetTarget) {
    ...
    deut_prm->fTargetPosition.SetXYZ(
      fTargetPos.x()/mm, 
      fTargetPos.y()/mm, 
      fTargetPos.z()/mm
    );
    deut_prm->fTargetAngle = -fTargetAngle;
    deut_prm->fTargetThickness = fTargetSize.z();

    if (fTargetSD==0){
      fTargetSD = new FragmentSD("/Target");
      SDMan->AddNewDetector(fTargetSD);
    }
    LogicTarget->SetSensitiveDetector(fTargetSD);
  }
...
//______________________________________________________________________________
void DayoneDetectorConstruction::SetTargetAngle(G4double angle)
{
  fTargetAngle = angle;
  std::cout<<"DayoneDetectorConstruction : Set target angle at  "
	   <<fTargetAngle/deg<<" deg  "
	   <<std::endl;
}
//______________________________________________________________________________
void DayoneDetectorConstruction::SetPDCAngle(G4double angle)
{
  fPDCAngle = angle;
  std::cout<<"DayoneDetectorConstruction : Set PDC angle at  "
	   <<fPDCAngle/deg<<" deg  "
	   <<std::endl;
}
//______________________________________________________________________________
void DayoneDetectorConstruction::SetPDC1Pos(G4ThreeVector pos)
{
  fPDC1Pos = pos;
  std::cout<<"DayoneDetectorConstruction : Set PDC1 position at  "
	   <<fPDC1Pos.x()/cm<<" cm  "
	   <<fPDC1Pos.y()/cm<<" cm  "
	   <<fPDC1Pos.z()/cm<<" cm  "
	   <<std::endl;
...
//______________________________________________________________________________
void DayoneDetectorConstruction::SetPDC2Pos(G4ThreeVector pos)
{
  fPDC2Pos = pos;
  std::cout<<"DayoneDetectorConstruction : Set PDC2 position at  "
	   <<fPDC2Pos.x()/cm<<" cm  "
	   <<fPDC2Pos.y()/cm<<" cm  "
	   <<fPDC2Pos.z()/cm<<" cm  "
	   <<std::endl;
...
//______________________________________________________________________________
void DayoneDetectorConstruction::SetDumpAngle(G4double angle)
{
  fDumpAngle = angle;
  std::cout<<"DayoneDetectorConstruction : Set beam dump angle at  "
	   <<fDumpAngle/deg<<" deg  "
	   <<std::endl;
}
//______________________________________________________________________________
void DayoneDetectorConstruction::SetDumpPos(G4ThreeVector pos)
{
  fDumpPos = pos;
  std::cout<<"DayoneDetectorConstruction : Set beam dump position at  "
	   <<fDumpPos.x()/cm<<" cm  "
	   <<fDumpPos.y()/cm<<" cm  "
	   <<fDumpPos.z()/cm<<" cm  "
	   <<std::endl;
